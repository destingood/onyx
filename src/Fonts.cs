...
{
    /// <summary>
    /// Charge les polices OFFICIELLES d'ONYX embarquées (Marcellus = display/logo « or gravé »,
    /// Ubuntu = titres/nav, Inter = corps) via PrivateFontCollection. Deux voies GDI+ :
    /// AddMemoryFont d'abord, puis AddFontFile (fichier extrait dans %TEMP%) en SECOURS —
    /// certaines fontes (Inter) échouent en mémoire mais passent par fichier. Repli automatique
    /// sur Segoe UI/Georgia si une police manque. Les blocs mémoire restent vivants pour la
    /// durée de l'app (requis par GDI+).
    /// </summary>
    internal static class Fonts
    {
        ...
        private static FontFamily _ubuntu, _ubuntuMed, _inter, _marcellus;
        ...
        private static void Init()
        {
            ...
            foreach (var f in new[] { "Ubuntu-Regular.ttf", "Ubuntu-Medium.ttf", "Ubuntu-Bold.ttf",
                                      "Inter-Regular.ttf", "Marcellus-Regular.ttf" })
                Load(f);
            ...
            _marcellus = Find("Marcellus", false) ?? Installed("Marcellus");
        }
        ...
        private static void Load(string name)
        {
            try
            {
                ...
                int before = FamilyCount();
                IntPtr p = Marshal.AllocCoTaskMem(data.Length);
                Marshal.Copy(data, 0, p, data.Length);
                try { _pfc.AddMemoryFont(p, data.Length); } catch { }      // voie GDI+ n°1
                ...
                // Voie GDI+ n°2 : si AddMemoryFont n'a PAS enrichi la collection (Inter…),
                // on extrait la fonte dans %TEMP% et on la charge par fichier — bien plus tolérant.
                if (FamilyCount() == before)
                {
                    string dir = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "onyx-fonts");
                    System.IO.Directory.CreateDirectory(dir);
                    string path = System.IO.Path.Combine(dir, name);
                    try { System.IO.File.WriteAllBytes(path, data); } catch { }   // déjà présent/verrouillé : on garde l'existant
                    try { _pfc.AddFontFile(path); _log.Add(name + "=fichier"); return; } catch { }
                }
                _log.Add(name + "=ok");
            }
        ...
        {
            try { return _pfc.Families.Length; } catch { return 0; }
        }
        ...
        public static FontFamily Marcellus { get { Init(); return _marcellus; } }
        ...
        public static string Diagnostic()
        {
            ...
            return "Polices résolues : Ubuntu=" + (_ubuntu != null ? _ubuntu.Name : "NON") +
                   ...
                   " · Marcellus=" + (_marcellus != null ? _marcellus.Name : "NON") +
                   "  [" + string.Join(",", _log) + "]";
        }
    }
}
