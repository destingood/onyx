    ...
    internal static class LibScan
    {
        ...
        public static List<LibItem> Items()
        {
            ...
            return new List<LibItem>
            {
                ...
                new LibItem { Name = "f.lux (confort visuel, moins de fatigue oculaire)", WingetId = "flux.flux",
                    Why = "adapte la température de couleur de l'écran à l'heure — repos des yeux lors des longues sessions de jeu",
                    Installed = () => Uninstall("f.lux") || WingetPkg("flux.flux") },
                new LibItem { Name = "Autoruns (gestionnaire de démarrage Sysinternals)", WingetId = "Microsoft.Sysinternals.Autoruns",
                    Why = "voit et coupe TOUT ce qui se lance au démarrage (bien plus complet que le Gestionnaire des tâches) — outil Microsoft officiel",
                    Installed = () => WingetPkg("Microsoft.Sysinternals.Autoruns") },
                new LibItem { Name = "ZoomIt (zoom & annotation d'écran, Sysinternals)", WingetId = "Microsoft.Sysinternals.ZoomIt",
                    Why = "zoom et dessin à l'écran pour présenter/streamer — l'outil de CAPET pour montrer un détail en direct",
                    Installed = () => WingetPkg("Microsoft.Sysinternals.ZoomIt") },
                ...
                // Ollama a été RETIRÉ de cette liste avec l'IA locale (voir LocalBrain.Retiree).
                // Sa seule justification ici était « le cerveau étendu du Copilote » : le
                // proposer encore ferait installer un serveur résident dont ONYX ne se sert
                // plus — et sur une machine dont tout le produit traque ce qui coûte des
                // microsecondes, c'est le pire conseil possible.
